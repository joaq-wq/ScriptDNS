...
# Função para instalar dialog se não existir
install_dialog() {
    case "$1" in
        ubuntu)
            ...
            ;;
        opensuse)
            sudo zypper refresh && sudo zypper install -y dialog
            ;;
        centos)
...
# Detecta o SO para instalar dialog e usar nas opções
detect_os() {
...
# Usando dialog para criar o menu de seleção do sistema operacional
OPCAO=$(dialog --clear --backtitle "Configurador de DNS" \
    --title "Escolha o sistema operacional" \
    --menu "Selecione a opção:" 15 50 4 \
    1 "Ubuntu" \
    2 "openSUSE" \
    3 "CentOS" \
    4 "Arch Linux" \
...
EOF
    ...
    3)
        ...
        echo "Atualizando pacotes..."
        echo $SENHA | sudo -S dnf update -y

        echo "Instalando BIND..."
        echo $SENHA | sudo -S dnf install -y bind bind-utils
